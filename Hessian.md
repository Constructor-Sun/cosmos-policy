...
最初考虑过使用 `libero_plus_10.zip` 中的 expert action 作为监督目标，但这个数据集只能确认是同一 base task 的 demonstrations，不能严格证明 Phase 1 的 `episode/init_state_index` 与 LeRobot demo episode 一一对应。

因此当前更稳妥的 loss 是 clean-success reference action loss：
...
但这个相关性只能作为 supporting evidence，不应作为独立因果证明。原因是 action deviation 和 Hessian loss 都使用同一个 clean-final-action reference，因此二者在定义上并非完全独立。
