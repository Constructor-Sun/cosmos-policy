...
COSMOS_ROLLOUT_SUBDIR=${COSMOS_ROLLOUT_SUBDIR:-08-31-curobo}
...
run_task() {
    ...
    mkdir -p "$task_output"
    echo "[K6] COSMOS_HELD_OBJECT=$COSMOS_HELD_OBJECT"
    ...
    SMOKE_RUN_ID="k6_held_object_gpu${ROBOTINIT_GPU}" \
...
printf 'K6 held-object evaluation completed.\nResults: %s\n' "$OUTPUT_ROOT/$RESULT_NAME"
