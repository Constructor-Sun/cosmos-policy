...
class HeldObjectPlanner:
    ...
    def __init__(
        ...
        attachment_slots: int = 64,
        attachment_padding: float = 0.002,
        enable_urdf_robot_filter: bool = True,
        removal_sphere_slots: int = 32,
        removal_sphere_padding: float = 0.005,
    ) -> None:
        self.robot = robot
        ...
        self.removal_sphere_slots = int(removal_sphere_slots)
        self.removal_sphere_padding = float(removal_sphere_padding)
    ...
    @staticmethod
    def _build_attachment_spheres(points_hand, max_slots, padding):
        """Cover observed object points with deterministic local spheres.

        The grid only partitions the input; unlike ``_build_spheres``, empty
        cells do not produce spheres and an occupied cell is covered using the
        bounds of its actual points rather than the full grid-cell diagonal.
        Every finite input point is therefore covered without filling the
        complete object AABB.
        """
        points = np.asarray(points_hand, dtype=np.float64).reshape(-1, 3)
        points = points[np.isfinite(points).all(axis=1)]
        max_slots = max(1, int(max_slots))
        ...
        cell = extent / grid.astype(np.float64)
        cell_indices = np.floor((points - lower) / cell).astype(np.int64)
        cell_indices = np.clip(cell_indices, 0, grid - 1)
        occupied = np.unique(cell_indices, axis=0)
        margin = max(float(padding), 1e-4)

        spheres = []
        for cell_index in occupied:
            cluster = points[np.all(cell_indices == cell_index, axis=1)]
            cluster_lower = cluster.min(axis=0)
            cluster_upper = cluster.max(axis=0)
            center = 0.5 * (cluster_lower + cluster_upper)
            radius = float(np.linalg.norm(cluster - center, axis=1).max() + margin)
            spheres.append([*center, radius])
        return np.asarray(spheres, dtype=np.float64).reshape(-1, 4)

    @staticmethod
    def _sphere_extent(spheres):
        spheres = np.asarray(spheres, dtype=np.float64).reshape(-1, 4)
        lower = np.min(spheres[:, :3] - spheres[:, 3, None], axis=0)
        upper = np.max(spheres[:, :3] + spheres[:, 3, None], axis=0)
        return upper - lower
    ...
    def plan(self, inp: HeldObjectPlannerInput) -> HeldObjectPlannerResult | None:
        ...
        if inp.depth is None or inp.camera_params is None or inp.joint_positions is None:
            self.last_failure = "missing depth/camera/joint_positions"
            print("[HeldObjectPlanner] missing depth/camera/joint_positions")
            logger.warning("HeldObjectPlanner: missing depth/camera/joint_positions")
            return None
        ...
        try:
            ...
            self.last_failure = f"cuRobo import failed: {exc}"
            print(f"[HeldObjectPlanner] cuRobo import failed: {exc}")
            logger.warning("HeldObjectPlanner: cuRobo import failed: %s", exc)
            return None
        try:
            ...
            removal_spheres_hand = self._build_spheres(
                points_hand,
                self.removal_sphere_slots,
                self.voxel_size,
                self.removal_sphere_padding,
            )
            attachment_spheres_hand = self._build_attachment_spheres(
                points_hand,
                self.attachment_slots,
                self.attachment_padding,
            )
            if removal_spheres_hand is None or attachment_spheres_hand is None:
                self.last_failure = "failed to build attachment spheres"
                print("[HeldObjectPlanner] failed to build attachment spheres")
                logger.warning("HeldObjectPlanner: failed to build attachment spheres")
                return None
            finite_points_hand = points_hand[np.isfinite(points_hand).all(axis=1)]
            point_extent = np.ptp(finite_points_hand, axis=0)
            removal_extent = self._sphere_extent(removal_spheres_hand)
            attachment_extent = self._sphere_extent(attachment_spheres_hand)
            attachment_radii = attachment_spheres_hand[:, 3]
            print(
                "[HeldObjectPlanner] attachment geometry "
                f"points={len(finite_points_hand)} "
                f"point_extent_mm={np.round(point_extent * 1000.0, 1).tolist()} "
                f"removal_spheres={len(removal_spheres_hand)} "
                f"removal_extent_mm={np.round(removal_extent * 1000.0, 1).tolist()} "
                f"attachment_spheres={len(attachment_spheres_hand)}/{self.attachment_slots} "
                f"attachment_extent_mm={np.round(attachment_extent * 1000.0, 1).tolist()} "
                "attachment_radius_mm="
                f"[{attachment_radii.min() * 1000.0:.1f}, "
                f"{attachment_radii.mean() * 1000.0:.1f}, "
                f"{attachment_radii.max() * 1000.0:.1f}]"
            )
            ...
            try:
                ...
                def attempt(distance):
                    ...
                    print(f"[HeldObjectPlanner] attempt backoff={distance:.4f} n_surface={len(surface_points)}")
                    ...
                    if conflict is None:
                        ...
                        if min_info is not None:
                            print(
                                f"[HeldObjectPlanner] success backoff={distance:.4f}: "

                                f"min_clearance={min_info.min_clearance:.4f} "

                                f"point={min_info.min_clearance_point.tolist()} "

                                f"sphere_center={min_info.min_clearance_sphere_center.tolist()}"
                            )
                        self.last_debug = {
                            "points_hand": points_hand,
                            "removal_spheres_hand": removal_spheres_hand,
                            "attachment_spheres_hand": attachment_spheres_hand,
                            ...
                            "last_conflict_info": (
                                {
                                    "point": min_info.min_clearance_point,
                                    "sphere_center": min_info.min_clearance_sphere_center,
                                    "clearance": min_info.min_clearance,
                                    "step": min_info.min_clearance_step,
                                }
                                if min_info is not None
                                else None
                            ),
                    ...
                    print(
                        f"[HeldObjectPlanner] backoff={distance:.4f}: path conflict "

                        f"first={conflict.first_step} last={conflict.last_step} "

                        f"clearance={conflict.min_clearance:.4f} "

                        f"point={conflict.min_clearance_point.tolist()} "

                        f"sphere_center={conflict.min_clearance_sphere_center.tolist()}"
                    )
                    return None
                selected = find_minimum_feasible_backoff(
                    attempt,
                    max_backoff=0.04,
                    min_backoff=-0.04,
                )
                if selected is None:
                    self.last_failure = "no feasible backoff to ready pose"
                    nearest_obstacle_to_target = None
                    if len(surface_points):
                        target_pos = np.asarray(inp.ready_pose, dtype=np.float64).reshape(6)[:3]
                        target_base = r_world_base.T @ (target_pos - t_world_base)
                        distances = np.linalg.norm(surface_points - target_base, axis=1)
                        idx = int(np.argmin(distances))
                        nearest_obstacle_to_target = {
                    ...
                    self.last_debug = {
                        ...
                        "t_world_base": t_world_base,
                        "target": np.asarray(inp.ready_pose, dtype=np.float64).reshape(6),
                        "last_conflict_info": last_conflict_info,
                        "nearest_obstacle_to_target": nearest_obstacle_to_target,
                    }
                    print("[HeldObjectPlanner] no feasible backoff to ready pose")
                    logger.warning("HeldObjectPlanner: no feasible backoff to ready pose")
                    return None
            ...
            self.last_failure = f"planning exception: {exc}"
            print(f"[HeldObjectPlanner] planning exception: {exc}")
