"""Sequential observation-only orchestration of the memory_system verifiers."""
...
from memory_system.types import CompletionResult, VerifierObservation
...
class ExecutionMonitor:
    ...
    def __init__(
        self,
        plans: dict[str, tuple[PhaseSpec, ...]],
        phase_verifier: Any,
        feasible_verifier: Any,
    ...
    def mark_wrong_grasp_recovery_finished(self, timestep: int | None = None) -> None:
        """After a wrong-object grasp recovery: return to the feasible gate."""
    ...
    def observe_vae(self, observation: VerifierObservation) -> ExecutionMonitorResult | None:
        ...
        if self.stage in {PHASE_CHECK, FEASIBLE_CHECK}:
            self.completion_verifier.calibrate(observation.main_vae)
            if hasattr(self.completion_verifier, "observe_wrist"):
                self.completion_verifier.observe_wrist(observation.wrist_image)
            if not (
                self.stage == FEASIBLE_CHECK and is_place(self.current_phase.skill)
            ):
                return None
        ...
        result = self.completion_verifier.update(
            observation.main_vae,
            gripper_closed=observation.gripper_closed,
            gripper_xy=observation.gripper_xy,
            target_bbox=self.target_bbox,
            timestep=observation.timestep,
            wrist_image=observation.wrist_image,
            gripper_qpos=observation.gripper_qpos,
        ...
        if (
            ...
            and observation.timestep is not None
            and observation.timestep - self.feasible_enter_timestep
    ...
    def observe(self, observation: VerifierObservation) -> ExecutionMonitorResult:
        if self.task_name is None:
        ...
        if before == FEASIBLE_CHECK and is_place(self.current_phase.skill):
            step_result = self.observe_vae(observation)
            if step_result is not None:
                return step_result

        if before == PHASE_CHECK:
            phase_result = self.phase_verifier.update(observation)
            ...
            if details.get("reason") == "no_templates":
                ...
                and phase_result.progress_px > 0
                and phase_result.target_xy is not None
                ...
                enough_ready = (
                    len(self.feasible_verifier.prototypes)
