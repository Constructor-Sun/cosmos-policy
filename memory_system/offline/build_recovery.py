...
def build_recovery(
    ...
    feasible_recovery_output: str | Path | None = None,
) -> int:
    """Build recovery / feasible-recovery / skill-memory artifacts.

    Library equivalent of the original ``bin/memory/build_vector_db_from_demos.py``
    CLI.  It preserves the model loading, T5 embedding cache, MuJoCo/HDF5 data
    reading, VAE encoding, and artifact writing behavior.
    """
    if input_dir is None:
        input_dir = ROOT / "LIBERO-Cosmos-Policy/success_only/libero_10_regen"
    input_dir = Path(input_dir).expanduser().resolve()
    if segments_manifest is not None:
        segments_manifest = Path(segments_manifest).expanduser().resolve()
    if output_dir is None:
        name = "skill_memory_demos" if segments_manifest is not None else "vector_db_demos"
        output_dir = ROOT / name
    output_dir = Path(output_dir).expanduser().resolve()
    output_dir.mkdir(parents=True, exist_ok=True)
    if feasible_recovery_output is not None:
        feasible_recovery_output = Path(feasible_recovery_output).expanduser().resolve()
    if device is None:
        device = "cuda" if torch.cuda.is_available() else "cpu"

    segment_map = (
        load_segment_manifest(segments_manifest)
        if segments_manifest is not None else None
    )
    cfg = SimpleNamespace(
        suite="libero",
    ...
    total = 0
    h5s = sorted(input_dir.glob("*_demo.hdf5"))
    recovery_targets: list[dict[str, Any]] = []
    feasible_recovery_targets: list[dict[str, Any]] = []
    if not h5s:
    ...
    for h5_path in h5s:
        ...
        with h5py.File(h5_path, "r") as handle:
            demos = sorted(
                handle["data"].keys(), key=lambda key: int(key.split("_")[1])
            )
        selected = demos[:max_per_task] if max_per_task > 0 else demos
        print(f"\n{task_name}: {len(selected)}/{len(demos)} demos", flush=True)
        for dk in selected:
            with h5py.File(h5_path, "r") as handle:
                group = handle["data"][dk]
                acts = group["actions"][:]
            ...
            T = acts.shape[0]
            demo_index = int(dk.split("_")[1])
            cache: dict[int, torch.Tensor] = {}

            def vae_at(frame: int) -> torch.Tensor:
                frame = min(max(int(frame), 0), T - 1)
                if frame not in cache:
                    cache[frame] = encode_vae(
                        frame, ajpg, wjpg, rstate, cfg, stats, t5_emb, model, dev
                    )
                return cache[frame]
            ...
            if segment_map is None:
                    ...
                    recovery_targets.append(
                        {
                            ...
                            "ee_states": np.asarray(
                                ee_states[recovery_frame], dtype=np.float32
                            ),
                            "action_scale": compute_action_scale(
                                acts, rstate, start, recovery_frame
                            ),
                            "action_chunk_raw": raw_action_chunk(
                                acts, start, start + 16
                            ),
                            "recovery_vae_main": vae_at(recovery_frame)[
                                :, 1:2, :, :
                            ].half(),
                        }
                    )
                        ...
                        feasible_recovery_targets.append(
                            {
                                ...
                                "ee_states": np.asarray(
                                    ee_states[ready_frame], dtype=np.float32
                                ),
                                "action_scale": compute_action_scale(
                                    acts,
                                    rstate,
                                    max(start, ready_frame - 16),
                                    ready_frame,
                                ),
                                "action_chunk_raw": raw_action_chunk(
                                    acts, ready_frame - 16, ready_frame
                                ),
                                "ready_vae_main": vae_at(ready_frame)[
                                    :, 1:2, :, :
                                ].half(),
                                "ready_vae_wrist": vae_at(ready_frame)[
                                    :, 0:1, :, :
                                ].half(),
                            }
                        )
                        ...
                        memory.update(
                            {
                                ...
                                "completion_sequence_source": (
                                    ...
                                    if success_end <= T
                                    else "truncated_success_window"
                                    if has_visual_success
                                    else "terminal_rgb_missing"
                                ),
                            }
                        )
                ...
                payload = {
                    "format": (
                        ...
                        if has_ready_boundaries
                        else "libero_skill_memory_v1"
                    ),
                    "task_name": task_name,
                    "demo_id": dk,
    ...
    print(f"\nDone. {total} chunks in {output_dir}", flush=True)
    return 0
