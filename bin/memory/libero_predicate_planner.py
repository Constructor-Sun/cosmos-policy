...
def main() -> int:
    ...
    parser.add_argument("--task", required=True, help="LIBERO task name, with or without .bddl")
    parser.add_argument("--engine", default="fast-downward", choices=("fast-downward", "fast-downward-opt"))
    ...
    try:
        ...
        problem, goals = build_problem(parsed, task_name)
    except (FileNotFoundError, ValueError, KeyError) as exc:
        print(f"ERROR: {exc}", file=sys.stderr)
        return 2

    print(f"Task: {task_name}")
    print(f"Language: {' '.join(parsed['language_instruction'])}")
    print("Goals: " + ", ".join("(" + " ".join(goal) + ")" for goal in goals))
    try:
        with OneshotPlanner(name=args.engine) as planner:
            result = planner.solve(problem)
    except Exception as exc:
        print(f"ERROR: planner {args.engine!r} is unavailable or failed: {exc}", file=sys.stderr)
        return 2
    if result.plan is None:
        print(f"No plan found (status={result.status.name}).", file=sys.stderr)
        return 1

    print(f"Plan ({len(result.plan.actions)} skills, status={result.status.name}):")
    for index, action_instance in enumerate(result.plan.actions, start=1):
        print(f"  {index}. {format_skill(action_instance)}")
    return 0
