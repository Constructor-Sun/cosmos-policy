#!/usr/bin/env python3
"""Minimal current/next phase state machine for the LIBERO phase verifier."""
...
try:
    from .libero_phase_verifier import (
        PHASE_ERROR,
        PHASE_UNKNOWN,
        LiberoPhaseVerifier,
        PhaseResult,
    ...
    from libero_phase_verifier import (  # type: ignore[no-redef]
        PHASE_ERROR,
        PHASE_UNKNOWN,
        LiberoPhaseVerifier,
        PhaseResult,
...
class LiberoPhaseMonitor:
    ...
    def __init__(
        ...
        **verifier_kwargs: Any,
        ...
        self.current_verifier = LiberoPhaseVerifier(self.memory, **verifier_kwargs)
        self.next_verifier = LiberoPhaseVerifier(self.memory, **verifier_kwargs)
    ...
    def _reset_verifier(self, verifier: LiberoPhaseVerifier, phase: PhaseSpec | None) -> None:
        if self.task_name is None:
    ...
    def observe(
        ...
        gripper_xy: tuple[float, float] | np.ndarray,
        ...
        current = self.current_verifier.update(third_view_rgb, gripper_xy)
        following = (
            self.next_verifier.update(third_view_rgb, gripper_xy)
