"""Render visible (RGB-D) point clouds for failure cases as PNG."""
import json, sys
...
OUTPUT_DIR = REPO / "rollouts/failure_pointcloud_visible_pngs"
...
def plot_pointcloud(points, path):
    ...
    fig.suptitle(f"{path.stem} visible N={len(points)}", fontsize=12)
    fig.tight_layout()
...
def main() -> int:
    ...
    from memory_system.pointcloud_action.offline.extraction import create_env, visible_point_cloud
    ...
    print(f"Rendering {len(failures)} visible point clouds to {OUTPUT_DIR}", flush=True)
    ...
    for row in failures:
        ...
        if obj is None:
            continue
        bddl_dir = REPO / "memory_system/pointcloud_action/generated_failure_visible_bddl"
        ...
        print("[render visible]", scene, row["object"], flush=True)
        try:
            ...
            try:
                ...
                pts = visible_point_cloud(env, obs, inst, 256)
                print("  points", len(pts), "bbox min", pts.min(axis=0) if len(pts) else None, "max", pts.max(axis=0) if len(pts) else None, flush=True)
                if len(pts) >= 4:
                    plot_pointcloud(pts, png_path)
                else:
                    print("  too few points", flush=True)
            finally:
                env.close()
