...
截至 2026-07-21，first-chunk grid 不再依赖 full-rollout summary。它读取已验证的扰动配置：

```text
experiments/paired_smoke_vla_jepa_kitchen_scene4_seed7_all_20case/seed_and_init_state_alignment_audit.json
```

该文件提供 camera perturbation 的变体定义：`view_50_0_100_0_0_initstate_0`、clean 使用 task instruction、perturbed 使用 base instruction。采集器从 LIBERO 三个 suite 的 `tasks_info.txt` 直接枚举 30 个 base task，并将该固定 suffix 追加到 base task 以构造扰动任务；因此不读取 full-rollout 标签或 episode summary。
...
因此为了和官方训练/测试分布更一致，当前 7500 first-chunk 生成暂时不扩展 env seed，只扩展 policy seed。扰动配置审计记录的历史 full rollout 使用 policy seed `7` 和 deterministic reset seed `0`；`7` 用于验证 camera 配置来源，grid 本身仍使用下述五个 policy seed。注意：`GetLatentSpecific.md` 中 20case 和 50case 的对比也显示 deterministic reset seed 会显著影响 group/result，所以这里固定 env/reset seed 是一个有意控制变量的选择，不表示 env seed 不重要。
...
- `source_group = null`：config 模式不读取旧 20-pair full-rollout 标签
- `init_state_index`
- `policy_seed`
- `env_seed`
- `seed_config_index`
...
GPU_IDS = 4 5 6 7
PERTURB_CONFIG = experiments/paired_smoke_vla_jepa_kitchen_scene4_seed7_all_20case/seed_and_init_state_alignment_audit.json
