"""Residual canonicalizers for paired Cosmos image latents."""
...
def _fixed_2d_encoding(height: int, width: int, dim: int) -> torch.Tensor:
    ...
    frequency = torch.exp(torch.arange(quarter, dtype=torch.float32) * scale)
...
class SampleGate(nn.Module):
    def __init__(
        self, channels: int, hidden_dim: int = 32, initial_gate: float = 0.12
    ) -> None:
        super().__init__()
...
class LatentCanonicalizer(nn.Module):
    ...
    def forward(
        ...
        if latent.ndim != 5 or tuple(latent.shape[1:]) != self.shape:
            expected = ",".join(map(str, self.shape))
            raise ValueError(f"expected [B,{expected}], got {tuple(latent.shape)}")
...
def load_model(
    ...
    model = LatentCanonicalizer(**checkpoint["model_config"])
