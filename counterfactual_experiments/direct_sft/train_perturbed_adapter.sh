...
OUT=$OUTPUT_ROOT/cosmos_policy/counterfactual_adapter_sft/light_libero10_500_transformer
...
python -m torch.distributed.run --nproc_per_node=4 --master_port=20477 \
  -m cosmos_policy.scripts.train --config="$CONFIG" -- \
  ...
  job.group=counterfactual_adapter_sft job.name=light_libero10_500_transformer
