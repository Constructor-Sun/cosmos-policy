#!/usr/bin/env python
"""Multi-view point cloud + trajectory, cropped to slightly larger than the main camera view.

This is a standalone diagnostic script.  It does not modify any production code.
It computes the 3D bounding box of the main-camera depth points, expands it by a
small margin, and only plots points inside that box.  This avoids the large
amount of far background / peripheral objects while still showing a little more
space than the raw main-camera view.
"""
...
def main():
    ...
    parser.add_argument("--crop-margin", type=float, default=0.12,
    ...
    parser.add_argument("--output", type=str,
                        default="tests/initial_alignment_investigation/initial_alignment_pointcloud_with_path_cropped_views.png")
    ...
    try:
        ...
        # Use a fixed workspace box that is slightly larger than the main-camera
        # central objects.  This removes far background / peripheral clutter.
        ...
        print(f"crop bounds lo={np.round(lo, 3)} hi={np.round(hi, 3)}")
        ...
        for panel, (view_name, elev, azim) in enumerate(VIEWS, start=1):
            ...
            ax.set_ylabel("y")
            ax.set_zlabel("z")
            ax.set_title(f"{view_name} (crop margin {args.crop_margin:.2f} m)", fontsize=11)
            ax.view_init(elev=elev, azim=azim)
            ax.legend(loc="upper right", fontsize=7)

        fig.suptitle(
            f"init_state_index={args.init_state_index} cropped point cloud + initial-alignment path",
            fontsize=15,
        )
        fig.tight_layout()
        ...
        print(f"saved cropped multi-view figure to {output_path}")
        if not args.no_show:
            plt.show()
        plt.close(fig)
    finally:
