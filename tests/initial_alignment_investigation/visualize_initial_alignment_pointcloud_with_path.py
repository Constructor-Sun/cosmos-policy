#!/usr/bin/env python
"""Combine the labeled 3D point cloud with the initial-alignment trajectory.

This is a standalone diagnostic script.  It does not modify any production code.
It shows:

- the RGB-D point cloud colored by object (MuJoCo segmentation oracle);
- the Curobo planned waypoint path;
- the actual EEF trajectory executed by WaypointPoseController;
- the memory target.
"""
...
def main():
    ...
    parser.add_argument("--output", type=str,
                        default="tests/initial_alignment_investigation/initial_alignment_pointcloud_with_path.png")
    ...
    try:
        ...
        for cat in CATEGORY_COLORS:
            ...
            ax.scatter(
                pts[:, 0], pts[:, 1], pts[:, 2],
                s=0.5, c=CATEGORY_COLORS[cat], alpha=0.5,
        ...
        ax.plot(
            ...
            "-", color="blue", linewidth=2.5, alpha=0.9, label="planned waypoints",
        )
        ax.scatter(
            waypoints[:, 0], waypoints[:, 1], waypoints[:, 2],
            s=12, c="blue", alpha=0.8,
        )

        # Actual EEF trajectory.
        ax.plot(
            actual_path[:, 0], actual_path[:, 1], actual_path[:, 2],
            "-", color="red", linewidth=2.5, alpha=0.9, label="actual EEF path",
        )
        ax.scatter(
            actual_path[:, 0], actual_path[:, 1], actual_path[:, 2],
            s=8, c="red", alpha=0.6,
        ...
        ax.scatter(
            ...
            s=80, c="magenta", marker="o", label="start",
        )

        ax.scatter(
            DEFAULT_TARGET[0], DEFAULT_TARGET[1], DEFAULT_TARGET[2],
            s=180, c="black", marker="*", label="memory target",
        )
        ...
        ax.set_zlabel("z (m)")
        ax.set_title(
            f"init_state_index={args.init_state_index} point cloud + initial-alignment path"
        )
